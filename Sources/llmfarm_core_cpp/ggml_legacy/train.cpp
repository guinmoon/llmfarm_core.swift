...
void sprint_duration(double fmillis,char* descr) {
    if (fmillis < 1000.0f) {
        sprintf(descr+strlen(descr),"%.1fms", (float) fmillis);
        return;
    }
    const int64_t one_sec  = 1000;
    const int64_t one_min  = one_sec  * 60;
    ...
    // to print int64_t either cast to (long long int) or use macro PRId64 from <inttypes.h>
    if (days > 0) {
        sprintf(descr+strlen(descr),"%lldd ", (long long int) days);
    }
    sprintf(descr+strlen(descr),"%02lld:%02lld:%02lld", (long long int) hours, (long long int) minutes, (long long int) seconds);
}
...
//            *sched, opt->loss_after);
        ...
        sprintf(descr, "%s: iter=%6d sample=%zu/%zu sched=%f loss=%f",
                __func__, opt->iter, std::min(1+train->shuffle_next_sample, train->shuffle_sample_count), train->shuffle_sample_count,
                *sched, opt->loss_after);
        *cancel = data->swiftcallback(descr);
        descr[0] = '\0';
        
        if (data->millis_per_iter > 0) {
//            printf(" dt=");
            auto res = sprintf(descr," dt=");
            sprint_duration(data->millis_per_iter,descr);
//            printf(" eta=");
            sprintf(descr+strlen(descr)," eta=");
            sprint_duration(remaining_millis,descr);
        }
