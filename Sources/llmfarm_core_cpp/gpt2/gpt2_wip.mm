...
struct gpt2_context * gpt2_init_from_file(const char * path_model, struct gpt_context_params   ctx_params, int n_gpu_layers) {

    ggml_dadbed9_time_init();

    gpt2_context * ctx = new gpt2_context;

    if (ctx_params.seed <= 0) {
        ctx_params.seed = time(NULL);
    }

    ctx->rng = std::mt19937(ctx_params.seed);
//    gpt2_model_load(const std::string & fname, gpt2_model & model, gpt_vocab & vocab, const gpt_params & params)
    gpt_params params;
    params.n_gpu_layers = n_gpu_layers;
    params.n_batch = ctx_params.n_batch;
    
    if (!gpt2_model_load(path_model, ctx->model, ctx->vocab,params)) {
    ...
    ctx->sched = ggml_backend_sched_new(ctx->model.backends.data(), ctx->model.backends.size());

    // create the worst case graph for memory usage estimation
    int n_tokens = std::min(ctx->model.hparams.n_ctx, params.n_batch);
    int n_past = ctx->model.hparams.n_ctx - n_tokens;
    ctx->gf = gpt2_graph(ctx->model, n_past, std::vector<gpt_vocab::id>(n_tokens, 0));

    ggml_backend_sched_init_measure(sched, ctx->gf);


    // compute the required memory
    size_t mem_size = 0;
    for (size_t i = 0; i < ctx->model.backends.size(); i++) {
        ggml_backend_buffer_t buf = ggml_backend_sched_get_buffer(ctx->sched, ctx->model.backends[i]);
        size_t size = ggml_backend_buffer_get_size(buf);
        if (size > 0) {
            mem_size += size;
            printf("%s: %8s compute buffer size = %8.2f MB\n", __func__, ggml_backend_name(ctx->model.backends[i]), size/1024.0/1024.0);
...
//    size_t align = ggml_backend_get_alignment(ctx->model.backend);
//    ctx->allocr = ggml_allocr_new_measure(align);
//    int n_tokens = std::min(ctx->model.hparams.n_ctx, params.n_batch);
//    int n_past = ctx->model.hparams.n_ctx - n_tokens;
//    struct ggml_cgraph * gf = gpt2_graph(ctx->model, ctx->allocr, n_past, std::vector<gpt_vocab::id>(n_tokens, 0));
//    size_t mem_size = ggml_allocr_alloc_graph(ctx->allocr, gf);
...
//            const std::vector<gpt_vocab::id> & embd_inp,
//                  std::vector<float>         & embd_w)
    if (!gpt2_eval(ctx->model,ctx->sched, 0, { 0, 1, 2, 3 }, ctx->logits)) {
...
//    gpt2_eval(ctx->model, n_threads, 0, { 0, 1, 2, 3 }, ctx->logits, mem_per_token);
    ...
    if (!gpt2_eval(ctx->model,ctx->sched,  n_past, embd, ctx->logits)) {
        fprintf(stderr, "%s: failed to eval\n", __func__);
        return 1;
    }
